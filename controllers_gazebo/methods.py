...
def goto(rxx,ryy,rphi,stop=False):
	...
	bnds=(bnd,bnd)
	#print bnds
	while (not rospy.is_shutdown()) and (np.abs(x-rx)>0.2 or np.abs(y-ry)>0.2):
	...
	while (not rospy.is_shutdown()) and np.sqrt ((x-rxx)**2+(y-ryy)**2)>0.05:#(np.abs(x-rxx)>0.04 or np.abs(y-ryy)>0.04):
